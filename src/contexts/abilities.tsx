...
import { getUser } from "@/config/auth/client-session";
...
export const AbilityProvider = ({ children }: { children: ReactNode }) => {
  const [useAuth, setUserAuth] = useState<IUser | null>(null);
  ...
  const can = (
    ...
    let userCan = user || useAuth;
    if (!userCan) return false;
    if (userCan.isAdmin) return true;

    let hasPermission = isIncluirString
  ...
  const canTabDefault = (
    ...
    let tab = tabs.find((tab) => can(tab, isIncluirString, customCheck)) || "";
    if (!tab) return "";
    return tab;
  };
